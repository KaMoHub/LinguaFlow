...
def review(request):
    ...
    from .services import get_cards_for_study_session, get_study_statistics, create_user_progress_for_card
    ...
    if request.method == 'POST':
        ...
        if not card_id:
            messages.error(request, 'Ошибка: не указана карточка')
        ...
        progress, created = UserWordProgress.objects.get_or_create(
            ...
            defaults={
                'status': 'new',
                'next_review_date': timezone.now()  # Сразу доступно для повторения
            }
        )
        ...
        messages.success(request, f'Карточка "{card.word_original}" обновлена!')
        return redirect('review')  # Перезагружаем для следующей карточки

    # Получаем статистику
    stats = get_study_statistics(user)
    review_count = stats['review_count']
    new_count = stats['new_count']

    # Получаем карточку для текущей сессии
    current_card = None
    is_new = False

    if session_type == 'review' and review_count > 0:
        # Берем первую карточку из тех, что нужно повторить
        review_cards = get_cards_for_study_session(user, 'review', limit=1)
        if review_cards:
            current_card = review_cards[0]
            is_new = False

    elif session_type == 'new' and new_count > 0:
        # Берем первую новую карточку
        new_cards = get_cards_for_study_session(user, 'new', limit=1)
        if new_cards:
